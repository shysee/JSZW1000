...
### 15.12 Rockwell PLC artifact boundary and metadata envelope

This pass re-checked the Rockwell project folder with repeatable scripts instead of only raw string scans. The folder currently contains four companions:

- `FolderDuplexGen3L30V33Rev3_9_PIO.ACD`
- `Folder_DuplexGen3_L30V33PIO_Rev3_7.ACD.Recovery`
- `Folder_DuplexGen3_L30V33PIO_Rev3_7.Sem`
- `Folder_DuplexGen3_L30V33PIO_Rev3_7.Wrk`

Recovered container-level facts:

- `.Sem` and `.Wrk` are both zero-byte placeholders in this SWI package, so they do not contribute any extra readable PLC layer here.
- both `.ACD` files still begin with the plain-text RSLogix 5000 banner:
  - `This file was generated by the RSLogix 5000 software.`
  - `DO NOT ALTER THIS FILE`
- the visible save history and object-family metadata remain readable from the main container body.
- the main `.ACD` exposes `37` gzip signatures and the `.ACD.Recovery` exposes `36`.

What this proves:

- these Rockwell artifacts are mixed containers, not only flat opaque binaries
- the readable layer includes both plain-text envelope data and many compressed subdocuments
- the companion `.Sem` / `.Wrk` files are not the missing logic source in this sample set

What this still does **not** prove:

- no ladder/ST routine bodies
- no rung order
- no full task scheduling graph

### 15.13 Embedded `LogixQuickInfo` / `LogixTagInfo` members and project symbol directory

This pass added `tools/swi_reverse/extract_rockwell_gzip.py` and used it to decompress the embedded gzip members inside the Rockwell project files.

Confirmed XML members from the main `.ACD`:

- `0x21985` -> `LogixQuickInfo Name="ProjectTemplate"`
- `0x21BBC` -> `LogixTagInfo Name="ProjectTemplate"`
- `0x10DAC9` -> `LogixQuickInfo Name="Folder"`
- `0x10DD0D` -> `LogixTagInfo Name="Folder"`

Confirmed XML members from `.ACD.Recovery`:

- `0x21211` -> `LogixQuickInfo Name="ProjectTemplate"`
- `0x21448` -> `LogixTagInfo Name="ProjectTemplate"`
- `0x10D355` -> `LogixQuickInfo Name="Folder"`

The recovery file also contains a small non-XML gzip member at `0x10D592` (`49` bytes), which means some of the compressed subdocuments are still compact binary records rather than text/XML.

Recovered project-level facts from `LogixQuickInfo Name="Folder"`:

- `SWVersion String="RSLogix 5000 v33.00"`
- `DeviceIdentity VendorID="1" ProductType="14" ProductCode="156" MajorRevision="33" MinorRevision="13"`
- `ControllerSlot SlotNumber="0"`
- `ChangeLog CurrentIndex="97"` in the main `.ACD`
- `ChangeLog CurrentIndex="0"` in `.ACD.Recovery`

Recovered project-level facts from `LogixTagInfo Name="Folder"` in the main `.ACD`:

- `DataTypes: 212`
- `Controller tags: 1028`
- `Programs: 1`
- the only visible program name in this tag directory is `MainProgram`
- `MainProgram` currently exposes two local tags in the readable tag directory:
  - `T_PumpA_IdleSpeed`
  - `T_PumpB_IdleSpeed`

Most important HMI-to-PLC symbol matches now confirmed directly from the PLC project tag directory:
...
- status / process values:
  ...
  - `NoOfOperators`
  - `AnglePeak`
- sequence / semi-auto state:
  - `a_StartStep`
  - `sa_StartStep`
  - `sa_BackGauge`

This sharpens the current PLC evidence boundary:

- the PLC side is no longer only a version/object-count envelope; the main `.ACD` does expose a project-level symbol directory offline
- the readable PLC layer now covers controller/program names, tag names, datatypes, and many HMI-facing symbol matches
- the readable PLC layer still does **not** expose rung bodies, routine source, or the exact execution order behind those tags
...
- RSLogix 5000 `.ACD` / `.ACD.Recovery` gzip-member extraction and XML parsing for `LogixQuickInfo` / `LogixTagInfo`
