...
namespace JSZW1000A
{
    public partial class MainFrm : Form
    {
        ...
        private void btn导航_自动_Click(object sender, EventArgs e)
        {
            ...
            // 显示部分面板和按钮
            pnl自动4视图.Visible = pnl角度尺寸.Visible = btn重置视图.Visible = true;
        ...
        private void btn条款确定_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MainFrm.Lang == 0)
                    MessageBox.Show("在启动之前，你必须完成所有列出的检查!");
                else
                    MessageBox.Show("Before Starup,The Studies Listed are Complete!");
            }
        ...
        public void 切入自动1(bool isCal, bool 需生成序列)     //是否计算长度角度
        {
            ...
            // 设置按钮可见性
            btn折弯预览.Visible = btn构图完成.Visible = false;
        ...
        private void 切入折弯预览(bool proc)      //true:折弯预览 false:折弯设置
        {
            ...
            // 设置按钮可见性
            btn折弯预览.Visible = btn重置视图.Visible = true;
        ...
        private void btn折弯预览_Click(object sender, EventArgs e)
        {
            ...
            {
                btn折弯预览.Text = " 折弯\r\n预览";
                btn折弯预览.Image = global::JSZW1000A.Properties.Resources.Preview;
        ...
        private void timer1s_Tick(object sender, EventArgs e)
        {
            ...
            lbMT9.Text = (lbMT9.Visible) ? Hmi_rArray[41].ToString("f1") : "0.0";
            ...
            for (int i = 0; i < 14; i++)
            {
                ...
                if ((Hmi_iArray[26] & p) != (lastErrCode26 & p) && ((Hmi_iArray[26] & p) == p))     //不等,且为1
                    msgAdd((MainFrm.Lang == 0) ? errMsg[0, i] : errMsg_en[0, i]);
            }
            ...
            for (int i = 0; i < 14; i++)
            {
                ...
                if ((Hmi_iArray[24] & p) != (lastWarnCode24 & p) && ((Hmi_iArray[24] & p) == p))
                    msgAdd((MainFrm.Lang == 0) ? warnMsg[0, i] : warnMsg_en[0, i]);
            }
            lastWarnCode24 = Hmi_iArray[24];

            if (Hmi_iArray[22] != lastTipCode22 && Hmi_iArray[22] > 0)
                msgAdd("♂ Msg : " + ((MainFrm.Lang == 0) ? TipMsg[0, Hmi_iArray[22]] : TipMsg_en[0, Hmi_iArray[22]]));
            ...
            if ((Hmi_iArray[22]) != (lastTipWinCode22) && Hmi_iArray[22] > 0)
            {
                ...
                FrmTips dlgTips = new FrmTips(this, ((MainFrm.Lang == 0) ? TipMsg[0, Hmi_iArray[22]] : TipMsg_en[0, Hmi_iArray[22]]));
                dlgTips.StartPosition = FormStartPosition.Manual;
                dlgTips.Location = new Point(500, 200);
                bTipFlag = true;
                dlgTips.ShowDialog();
        ...
        {
            FrmTips dlgTips = new FrmTips(this, ((MainFrm.Lang == 0) ? TipMsg[i, j] : TipMsg_en[i, j]));
            dlgTips.StartPosition = FormStartPosition.Manual;
            dlgTips.Location = new Point(500, 200);
            dlgTips.ShowDialog();
        }
        ...
        string[,] errMsg_en = {
            {
            "★ Err : Pull Rope Limit Wwitch Triggered____",
            "★ Err : Oil Pump Blockage Alarm____",
            "★ Err : Lubrication Pump Blockage Alarm____",
            "★ Err : Emergency Stop of Operation Cabinet____",
            "★ Err : Emergency Stop of Control Cabinet____",
            " code1",
            " code1",
            " code1",
            " code1",
        ...
        string[,] warnMsg_en = {
            {
            "☆ Warn: Top Apron, Fold isn't at the Origin, Sliding is Banned",
            "☆ Warn: Top Apron，Slide isn't at the Working Point, Folding is Banned",
            "☆ Warn: Btm Apron, Fold isn't at the Origin, Sliding is Banned",
            "☆ Warn: Btm Apron，Slide isn't at the Working Point, Folding is Banned",
            "☆ Warn:   ",
            "☆ Warn:   ",
            "☆ Warn:   ",
            " code1"," code1"," code1"," code1"," code1"," code1"," code1",
        ...
        string[,] TipMsg_en = {
            {
             " ",
             "Unlocking the Backgauge",
             "Locking the Backgauge",
             "Locking the Clamp",
             "Pedal on Fold/Start",
             "Pedal to Extend the Table",
             "Pedal to Retract the Table",
             "Operate the Slitting Knife",
             " code1"," code1"," code1"," code1"," code1",
             " code1","code1","code1"
            },
            { "The Slitting Knife Switch and Slitting Task must be Set together","code1","code1","code1","code1","code1","code1","code1","code1","code1","code1","code1","code1","code1","code1","code1"}
            };
