...
namespace JSZW1000A.SubWindows
{
    public partial class SubOPAutoDraw : UserControl
    {
        ...
        public SubOPAutoDraw(MainFrm fm1)
        {
            ...
            txb网格单元长.KeyDown += txb网格单元长_KeyDown;
            txb网格单元长.Leave += txb网格单元长_Leave;
        }
        ...
        private void setLang()
        {
            ...
            label15.Text = Strings.Get("AutoDraw.SnapToGrid");
            label2.Text = Strings.Get("AutoDraw.GridUnitLength");
            lb网格状态.Text = LocalizationText.OnOff(false);
            label3.Text = MainFrm.GetLengthUnitLabel();
            label4.Text = Strings.Get("AutoDraw.Fine");
            lb分条开.Text = Strings.Get("AutoDraw.Medium");
            label5.Text = Strings.Get("AutoDraw.Coarse");
            label6.Text = Strings.Get("AutoDraw.GridSize");
            btn清除.Text = Strings.Get("AutoDraw.Clear");
            btn撤销.Text = Strings.Get("AutoDraw.Undo");
        }
        ...
        //        // 2. 核心计算：长度 与 绝对角度
        //        // ---------------------------------------------------------
        //        List<double> absAngles = new List<double>(); // 临时存储每一段的绝对角度
        ...
        //            // B. 计算绝对角度 (弧度)
        //            // 注意：屏幕坐标系 Y 轴向下，所以用 Atan2(dy, dx) 算出来的角度，顺时针是正的
        //            double radian = Math.Atan2(dy, dx);
        //            absAngles.Add(radian * (180.0 / Math.PI)); // 转为角度存入临时列表
        //        }
        ...
        //            double currAbs = absAngles[i];     // 当前段绝对角度
        //            double prevAbs = absAngles[i - 1]; // 前一段绝对角度

        //            // A. 计算偏转角 (Deflection Angle)
        //            double diff = currAbs - prevAbs;

        //            // B. 归一化到 -180 ~ 180 之间
        //            // 这一步是为了处理跨越 180/-180 度临界点的情况
        //            while (diff > 180) diff -= 360;
        //            while (diff <= -180) diff += 360;

        //            // C. 计算工件内角 (Machine Angle)
        //            // 逻辑：直线是180度，偏转多少度，内角就减去多少度
        ...
        public bool ExecuteCalculation()
        {
            if (MainFrm.QuickDrawList.Count < 2)
            {
                MessageBox.Show(Strings.Get("AutoDraw.Error.DrawProfileFirst"));
                return false;
            }
            ...
            {
                MessageBox.Show(string.Format(Strings.Get("AutoDraw.Error.Calculation"), ex.Message));
                return false;
            }
        }
        private void timer1_Tick(object sender, EventArgs e)
        {
            sw对齐网格.BackgroundImage = bAlignGrid ? global::JSZW1000A.Properties.Resources.sw_左右小开关1 : global::JSZW1000A.Properties.Resources.sw_左右小开关0;
            lb网格状态.Text = LocalizationText.OnOff(bAlignGrid);
